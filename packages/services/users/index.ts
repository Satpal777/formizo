import {
  CreateUserWithEmailAndPasswordInputT,
  CreateUserWithEmailAndPasswordOutputT,
  ForgotPasswordInputT,
  ForgotPasswordOutputT,
  RefreshTokenInputT,
  RefreshTokenOutputT,
  ResetPasswordInputT,
...
export class UserService {
  ...
  private async sendVerificationEmailSafely(input: {
    email: string;
    name: string;
    userId: string;
    token: string;
  ...
  public async createUserWithEmailAndPassword(
    input: CreateUserWithEmailAndPasswordInputT,
  ): Promise<CreateUserWithEmailAndPasswordOutputT> {
    ...
    const emailSent = await this.sendVerificationEmailSafely({
      email,
      name,
      userId: newUser.id,
      token: rawToken,
  ...
  public async verifyEmail(input: VerifyEmailInputT) {
    ...
    const user = await db
      .select()
      .from(users)
      .where(and(eq(users.verificationToken, hashedToken), eq(users.id, id)))
      .limit(1)
      .then((result) => result[0]);
  ...
  public async resetPassword(input: ResetPasswordInputT): Promise<ResetPasswordOutputT> {
    ...
    const user = await db
      .select()
      .from(users)
      .where(and(eq(users.id, id), eq(users.forgotPasswordToken, hashedToken)))
      .limit(1)
      .then((result) => result[0]);
  ...
  public async signInWithEmailAndPassword(
    input: signInWithEmailAndPasswordInputT,
  ): Promise<signInWithEmailAndPasswordOutputT> {
